 ...
 */
public class XmlInputFormat extends TextInputFormat implements JobConfigurable {
    ...
    @Override
    public RecordReader<LongWritable, Text> getRecordReader(InputSplit inputSplit, JobConf _usingConstructor, Reporter reporter) throws IOException {
        return new XmlRecordReader((FileSplit) inputSplit, this.jobConf);
    }
    ...
    public static class XmlRecordReader implements RecordReader<LongWritable, Text> {
        ...
        public XmlRecordReader(FileSplit input, JobConf jobConf) throws IOException {
            ...
            this.startTag = findProperty(jobConf, START_TAG_KEY).getBytes("utf-8");
            this.endTag = findProperty(jobConf, END_TAG_KEY).getBytes("utf-8");
        ...
        private static String findProperty(JobConf jobConf, String propertyName) {
            ...
            // TBLPROPERTIES are copied to JobConf properties, or they are not.
            String jobConfProperty = jobConf.get(propertyName);
            if (jobConfProperty != null) {
                return jobConfProperty;
            }

            MapWork mapWork = Utilities.getMapWork(jobConf);
            Map<String, PartitionDesc> map = mapWork.getPathToPartitionInfo();
            for (Map.Entry<String, PartitionDesc> pathsAndParts : map.entrySet()) {
                Properties props = pathsAndParts.getValue().getProperties();
                String prop = props.getProperty(propertyName);
                if (prop == null) {
                    throw new IllegalArgumentException("Cannot find property: " + propertyName + " Was looking it from partitioninfo of MapWork.");
                }
                return prop;
            }
            throw new RuntimeException("mapWork partition info was empty. Cannod find property: " + propertyName);
        }
